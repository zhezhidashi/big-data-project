...
for thread_count in thread_counts:
    ...
    # 建立重定向输出目录的文件夹
    output_dir = os.path.join("test06", f"thread-{thread_count:02d}-result")
    script_dir = os.path.join("test06", f"thread-{thread_count:02d}")
    os.makedirs(output_dir, exist_ok=True)
    os.makedirs(script_dir, exist_ok=True)

    
    script_file = os.path.join(script_dir, f"auto-test06-{thread_count:02d}.sh")
