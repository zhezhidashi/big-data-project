    ...
    for i in range(len(thread_counts)):
        ...
        while end_index < len(sql_files):
            ...
            for sql_file in selected_sql_files:
                ...
                script += f"/root/trino/trino-01/trino-cli --server localhost:8090 -f {sql_file} --catalog hive --schema aitopalluxio\n"

                for i in range(30):
                    for j in range(50):
                        script += "cp /mnt/fuse/result_test05_" + sql_file[8:10] + "_{:02d}/* /root/zzh/aitop-test/test-result_".format(i) + sql_file[8:10] + "_{:02d}_{:02d}.csv\n".format(i, j)
