    ...
    for i in range(len(thread_counts)):
        ...
        while end_index < len(sql_files):
            ...
            for sql_file in selected_sql_files:
                ...
                script += f"/root/trino/trino-server-429/trino-cli --server localhost:8090 -f {sql_file} --catalog hive --schema aitopalluxio\n"

                for i in range(30):
                    for j in range(50):
                        script += "cp /mnt/fuse/aitop-test06/result_test06_" + sql_file[8:10] + "_{:02d}/* /root/zzh/aitop-test/test-result_".format(i) + sql_file[8:10] + "_{:02d}_{:02d}.csv\n".format(i, j)
...
output_dirs = ["test06/thread-01", "test06/thread-02", "test06/thread-05", "test06/thread-10", "test06/thread-20"]
